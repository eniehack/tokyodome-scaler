<script lang="ts">
	...
	import ml, { type MapLayerMouseEvent, type MapLayerTouchEvent } from 'maplibre-gl';
	...
	const fetchTokyoDome = async (): Promise<GeoJSON.FeatureCollection<Polygon>> => {
		const resp = await fetch(`${base}/vatican_city.json`);
	...
	<button
		...
		onclick={() => {
			...
			for (let i = index; 0 <= i; i--) {
				let linelayer = `tokyodome${i}_linelayer`;
				if (typeof map.getLayer(linelayer) !== 'undefined') {
					map.removeLayer(linelayer);
				}
				let fillLayer = `tokyodome${i}_filllayer`
				if (typeof map.getLayer(fillLayer) !== 'undefined') {
					map.off('mousedown', fillLayer, fillLayerOnMouseDown);
					map.off('mouseup', fillLayer, fillLayerOnMouseUp);
					map.off('mousemove', fillLayer, fillLayerOnMouseMove);
					map.off('touchstart', fillLayer, fillLayerOnMouseDown);
					map.off('touchend', fillLayer, fillLayerOnMouseUp);
					map.off('touchmove', fillLayer, fillLayerOnMouseMove);
